...
export default function Carrito() {
  const [items, setItems] = useState<Item[]>(productosIniciales)
    ...
    <main style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          color: '#2C1A0E', marginBottom: '24px',
        }}>
          Tu carrito 🛒
        </h2>
        ...
        <div style={{ marginBottom: '24px' }}>
          {items.map(item => (
            <div key={item.id} style={{
              ...
              marginBottom: '12px', boxShadow: '0 2px 12px rgba(44,26,14,0.06)',
              display: 'flex', gap: '16px', alignItems: 'center',
            }}>
              <div style={{ fontSize: '40px' }}>{item.emoji}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: '700', color: '#2C1A0E', fontSize: '15px' }}>{item.nombre}</div>
                <div style={{ fontSize: '12px', color: '#8A7A6A' }}>{item.productor} · {item.unidad}</div>
        ...
        <div style={{
          ...
          boxShadow: '0 2px 12px rgba(44,26,14,0.06)', marginBottom: '16px',
        }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <span style={{ fontWeight: '600' }}>${envio.toLocaleString()}</span>
          </div>
          <div style={{
            borderTop: '1px solid rgba(44,26,14,0.08)', paddingTop: '16px',
            display: 'flex', justifyContent: 'space-between',
            fontWeight: '900', fontSize: '20px',
          }}>
            <span>Total</span>
            <span style={{ color: '#C8842A' }}>${total.toLocaleString()}</span>
          </div>
        </div>
        ...
        <p style={{
          ...
          fontSize: '12px', color: '#8A7A6A', marginTop: '12px',
        }}>
          🔒 Pago seguro · Envío a todo Colombia
        </p>

        <div style={{ textAlign: 'center' as const, marginTop: '20px' }}>
          <Link href="/catalogo" style={{ color: '#8A7A6A', fontSize: '14px' }}>
            ← Seguir comprando
          </Link>
        </div>
      </div>
